// =====================================================================
// PropriedadesScreen — Imóveis: Construir · Gerenciar · Aluguéis
// =====================================================================
...
export function PropriedadesScreen({ gameState, api, onSpend, onReceive }: PropriedadesScreenProps) {
  const [tab,         setTab]         = useState<'construir' | 'meus' | 'alugueis'>('meus');
  ...
  // ── Tab bar ────────────────────────────────────────────────────
  const TABS = [
    { id: 'meus'      as const, label: 'Meus Imóveis', badge: activeProps.length },
    { id: 'construir' as const, label: 'Construir',    badge: null },
    { id: 'alugueis'  as const, label: 'Aluguéis',     badge: rentedProps.length },
  ];
        ...
        {buildingNow.length > 0 && (
          <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-400/30 rounded-xl px-3 py-2">
            ...
            <span className="text-[12px] text-amber-700 font-semibold">
              {buildingNow.length} obra{buildingNow.length > 1 ? 's' : ''} em andamento
            </span>
          </div>
        )}
      </div>
                    ...
                    <div className="border-t border-border px-3 pb-4 pt-3 space-y-3">
                      ...
                      {prop.status === 'alugado' && (
                        <div className="space-y-2">
                          ...
                          {/* Also allow listing for sale while rented */}
                          ...
                          {prop.pendingBuyerName ? (
                            <>
                              ...
                              <div className="flex gap-2">
                                ...
                                <Button size="sm" variant="outline" onClick={() => handleReject(prop.instanceId)}>
                                  Recusar
                                </Button>
                              </div>
                            </>
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <Button size="sm" variant="outline" onClick={() => cancelSale(prop.instanceId)}>
                                Cancelar
                              </Button>
                            </div>
                          )}
            ...
            {catalogFiltered.map(build => {
              ...
              return (
                <button
                  ...
                  className={`w-full ios-surface text-left flex items-start gap-3 p-3 transition-all ${
                    sel ? 'ring-2 ring-primary' : ''
                  } ${!info.canBuild ? 'opacity-60' : ''}`}
                >
          ...
          {buildTypeId && buildInfo && (() => {
            ...
            return (
              <div className="ios-surface p-4 space-y-3">
                ...
                {!canAfford && (
                  <div className="flex items-center gap-2 bg-destructive/10 border border-destructive/30 rounded-xl px-3 py-2">
                    ...
                    <span className="text-[12px] text-destructive">
                      Falta {fmtBRL(buildInfo.totalCost - gameState.money)}
                    </span>
                  </div>
                )}
                {!buildInfo.canBuild && (
                ...
                <Button
                  className="w-full"
                  disabled={!canAfford || !buildInfo.canBuild}
                  onClick={handleBuild}
                >
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 space-y-0.5">
                        <div className="text-[12px]">
                          Pendente: <span className="font-bold text-foreground">{fmtBRL(pending)}</span>
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          Total: <span className="text-green-600 font-semibold">{fmtBRL(prop.rentCollected)}</span>
                        </div>
                      </div>
